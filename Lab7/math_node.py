...
class MathNode:
    ...
    def emit(self):
        ...
        :return:
        """
        return '(' + self.left.emit() + ' ' + str(self.token) + ' ' + self.right.emit() + ')'
    ...
    def evaluate(self):
        ...
        ret = 0;
        if self.token == '+':
            ret = self.left.evaluate() + self.right.evaluate()
        elif self.token == '*':
            ret = self.left.evaluate() * self.right.evaluate()
        elif self.token == '-':
            ret = self.left.evaluate() - self.right.evaluate()
        elif self.token == '//':
            if self.right.evaluate() is 0:
                raise runtime_error.RuntimeError('Division by zero error at line number ' + str(self.lineNum))
            else:
                ret = self.left.evaluate() // self.right.evaluate()
        return ret
